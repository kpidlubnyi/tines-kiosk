    ...
    <div class="cross-category-nav">
      <div class="cross-category-items">
        <button
          ...
          :key="catKey"
          class="cross-nav-btn"
          :class="{ 'is-active': catKey === currentPrefix }"
...
export default {
  ...
  props: {
    ...
    // Повний ключ поточної категорії, наприклад: "category1-x-y-z"
    activeCategory: {
      type: String,
      default: ''
    }
  ...
  computed: {
    ...
    // Отримуємо префікс поточної категорії (перша частина до дефісу)
    currentPrefix() {
      if (!this.activeCategory) return ''
      return this.activeCategory.split('-')[0]
    },
    // Отримуємо суфікс/кінцівку поточної категорії (все, що йде після першого дефісу)
    categorySuffix() {
      if (!this.activeCategory) return ''
      const firstDashIndex = this.activeCategory.indexOf('-')
      return firstDashIndex !== -1 ? this.activeCategory.slice(firstDashIndex) : ''
    }
  },
  methods: {
    getCategoryIcon(key) {
...
selectCrossCategory(targetCatKey) {
  if (targetCatKey !== this.currentPrefix) {
    this.$emit('select-cross-category', targetCatKey)
  }
}  }
}
</script>
