...
export default {
  ...
  beforeUnmount() {
    window.removeEventListener('hashchange', this.checkHashNavigation)
  },
  methods: {
    checkHashNavigation() {
      ...
      let categoryId = parts[0]
      let itemId = parts[1] || null

      if (!this.offersData[categoryId]) {
        itemId = categoryId
        categoryId = null

        for (const catKey in this.offersData) {
          const cat = this.offersData[catKey]
      ...
      if (categoryId && this.offersData[categoryId]) {
        this.activeOfferId = categoryId
        this.isOfferActive = true

        if (itemId) {
          const categoryItems = this.offersData[categoryId].items || []
    ...
    handleCarouselSelect({ item, event }) {
      ...
      setTimeout(() => {
        ...
        if (this.selectedItem) {
          window.location.hash = `${targetCategory}/${this.selectedItem.id}`
        }
      }, 100)
    ...
    openItemDetail({ item, event }) {
      ...
      setTimeout(() => {
        ...
        if (this.activeOfferId && item?.id) {
          window.location.hash = `${this.activeOfferId}/${item.id}`
        }
      }, 100)
    ...
    closeItemDetail() {
      ...
      setTimeout(() => {
        ...
        if (this.activeOfferId) {
          window.location.hash = this.activeOfferId
        }
      }, 100)
    ...
    handleCrossCategorySwitch(targetCategoryKey) {
      ...
      const suffixOf = id => id.slice(id.indexOf('-'));
      ...
      setTimeout(() => {
        ...
        if (this.isItemDetailActive && previousItem) {
          const newItems = this.offersData[targetCategoryKey]?.items || [];
          const matchedItem = newItems.find(
            item => suffixOf(item.id) === suffixOf(previousItem.id)
          );

          if (matchedItem) {
            this.selectedItem = matchedItem;
            window.location.hash = `${targetCategoryKey}/${matchedItem.id}`;
          } else {
            this.isItemDetailActive = false;
            this.isContentCollapsed = false;
            this.selectedItem = null;
            window.location.hash = targetCategoryKey;
          }
        } else {
          window.location.hash = targetCategoryKey;
        }
